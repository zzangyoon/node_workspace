...
var server = http.createServer(function(request, response){
    response.writeHead(200, {"Content-Type" : "text/html; charset=utf-8"});     //설정정보

    //클라이언트의 요청에 대한 응답 처리 (html 문서를 주고받음)

    //클라이언트의 요청내용이 다양하므로, 각 요청을 처리할 조건문이 있어야 한다
    //따라서 클라이언트가 원하는 것이 무엇인지부터 파악해야 한다
    ...
    var result = url.parse(request.url, true);    //url 모듈을 이용하여 전체 주소를 대상으로 해석 시작
                                                                  //true라고 옵션을 주면, 파라미터를 각각 접근할 수 있게 분리해줌

    //parsing한 결과인 result를 확인해보자
    //console.log("parsing 결과 : ", result);
