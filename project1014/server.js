...
var ejs = require("ejs");   //node 서버에서만 실행가능한 문서
                                      //html로 채워졌다고 해서 html 문서로 보면 안된다
...
var con;    //mysql 접속정보를 가진 객체, 이 객체로 sql 문을 수행할 수 있다


//서버 객체 생성
var server = http.createServer(function(request, response){

    //클라이언트가 원하는 요청을 처리하려면, URL을 분석하여 URI 추출해서 조건을 따져보자
    ...
    if(urlJson.pathname=="/"){
        fs.readFile("./index.html", "utf-8", function(error, data){
            if(error){
                ...
                //200이란?? 처리 성공 HTTP 상태코드, 404 : 존재하지 않는 자원, 500 : 서버의 심각한 에러
                response.writeHead(200, {"Content-Type" : "text/html; charset=utf-8"});     //편지봉투 작성
                response.end(data);       //편지지 작성
            }
        });
    ...
    }else if(urlJson.pathname=="/member/regist"){   //mysql 연동하여 insert
            ...
            if(error){
                ...
                con.query(sql, function(error, record, fields){    //select 쿼리문 수행
                    if(error){
                        ...
                        console.log("record : ", record);   //record : sql 결과문
        ...
        con.query(sql, function(error, record, fields){      //mysql에서 데이터가져오기
            if(error){
                console.log("회원 1건 조회 실패", error);
            }else{
                //console.log("2번 회원의 정보 : ", record);
                ...
                fs.readFile("./detail.ejs", "utf-8", function(error, data){
                    if(error){
                        ...
                        response.end(ejs.render(data, {
                            member:obj  //json 자체를 보내버림  >   결과를 받을때 member.~ 로 받아야함
                        })); //그냥 보내지 말고, 서버에서 실행할 후 그 결과를 보내자
                    }
                });
            }
        });
...
function getConnection(){
    con = mysql.createConnection(conStr); //json을 매개변수로 넣어주면 된다
}
